...
START_TEST(test_small_hexagon) {
    ...
    luary_dumpfxyzw(log, vertices, "vertices", 10);
    ...
    luary_dumpvoid(log, offsets, "offsets", 10);
    ...
    luary_dumpuint32(log, indices, "indices", 10);
    ...
    luary_dumpuint32(log, counts, "counts", 10);
    ...
    ck_assert(!luary_freevoid(&offsets, 0));
    ck_assert(!luary_freeuint32(&counts, 0));
    ck_assert(!luary_freefxyzw(&vertices, 0));
...
START_TEST(test_norms) {
    ...
    luary_dumpvnorm(log, vertices, "vertices", 10);
...
//    ck_assert(vertices->v[2].v.w == 1);
    luary_dumpint32(log, offsets, "offsets", 10);
    ck_assert_msg(offsets->mem.used == 2, "Expected 2 offsets, found %zu", offsets->mem.used);
    ...
    ck_assert(counts->i[0] == 5);
    ck_assert(counts->i[1] == 5);
    ck_assert(!luary_freeint32(&offsets, 0));
    ck_assert(!luary_freeuint32(&counts, 0));
    ck_assert(!luary_freevnorm(&vertices, 0));
    ck_assert(!log->free(&log, 0));
} END_TEST
